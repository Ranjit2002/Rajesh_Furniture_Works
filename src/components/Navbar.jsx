...
export default function Navbar() {
  ...
  // Base Desktop Link styling with your original hover line fix
  ...
  const MobileNavLink = ({ to, children }) => (
    <NavLink 
      ...
      className={({ isActive }) => `font-semibold tracking-widest text-lg w-full text-center transition-colors duration-300 ${
        isActive ? 'text-emerald-400 font-black drop-shadow-[0_0_8px_rgba(52,211,153,0.6)]' : 'text-gray-400 hover:text-white'
      }`}
    >
      {children}
    </NavLink>
    ...
    >
      <div className="mx-auto px-6 py-2 flex justify-between items-center transition-all duration-300">
        ...
        <div className="hidden md:flex space-x-8 lg:space-x-10 items-center">
          ...
          {/* DESKTOP THEME TOGGLE (Restored Previous SVG Icons) */}
          <button 
            onClick={toggleTheme} 
          ...
          >
            {isLightMode ? (
              /* Moon Icon (Light mode is active, click for Dark) */
              ...
              /* Sun Icon (Dark mode is active, click for Light) */
        ...
        <button 
          ...
          className="md:hidden relative z-20 text-gray-300 hover:text-white focus:outline-none p-2 transition-transform duration-300 hover:scale-110 cursor-pointer"
        >
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden absolute top-full left-0 w-full bg-gray-950/95 backdrop-blur-3xl border-b border-emerald-500/30 shadow-2xl flex flex-col items-center py-8 space-y-6">
          <MobileNavLink to="/">HOME</MobileNavLink>
          <MobileNavLink to="/about">ABOUT</MobileNavLink>
          <MobileNavLink to="/collections">COLLECTIONS</MobileNavLink>
          <MobileNavLink to="/contact">CONTACT</MobileNavLink>
          
          {/* MOBILE THEME TOGGLE (Restored Previous SVG Icons) */}
          <button 
            onClick={() => { toggleTheme(); setIsMobileMenuOpen(false); }} 
            className="theme-toggle px-8 py-3 rounded-full bg-gradient-to-r from-emerald-500 to-cyan-500 text-gray-950 text-sm font-black tracking-wider shadow-[0_0_15px_rgba(52,211,153,0.4)] flex items-center justify-center gap-2 cursor-pointer w-[60%]"
          >
            {isLightMode ? (
              <svg className="theme-toggle-light-icon w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z"></path>
